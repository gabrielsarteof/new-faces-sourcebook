---
id: common.skill.taichi
title: "Taichi"
version: 1.3
layer: scenario
scenario: common
type: skill
status: final
source-file: Pericia_Taichi_v1.md
---

PERÍCIA DE TAICHI
太極拳 · O ofício da força devolvida

O Taichi é a arte de não opor força a força. O praticante formado não bloqueia o golpe que chega, ele sente a direção, a intensidade e a intenção pelo contato, e devolve o que recebeu usando o próprio corpo do agressor como veículo. A postura enraizada absorve o que a estrutura montada não consegue deter, a torção de quadril redireciona o que a estrutura não precisa deter, e o excedente se guarda para sair de uma vez quando o praticante decidir. Nenhum dos três pilares é potência bruta, e é essa recusa que separa o domínio de qualquer arte que vença pelo músculo.

O território termina onde o ofício alheio começa. A definição dos quatro graus de estado e a régua que os posiciona pertencem ao Sistema de Efeitos de Status. A Energia de um golpe, a Guarda que o detém, a Absorção que o corpo sustenta e as quatro manobras de desequilíbrio pertencem ao Núcleo de Combate, que esta perícia consome inteiro sem redefinir uma linha dele. Os dois são camada de núcleo, e valem para qualquer cenário que hospede esta perícia. Obras nomeadas de escola de combate específica pertencem a compêndio próprio, ainda inexistente.

Esta perícia mora em worlds/common porque nenhuma linha dela pressupõe fato de cenário algum, e qualquer mundo que publique o Núcleo de Combate e o Sistema de Efeitos de Status pode hospedá-la sem tradução. No cenário Naruto, dois territórios vizinhos já publicados por outras perícias cruzam o caminho do Taichi e são cedidos: toda leitura de posição, identidade e intenção à distância pertence ao Sensoriamento, e repulsão carregada por chakra, seja rider de técnica, sopro elemental ou correnteza, pertence ao Controle de Chakra, caminho Impregnação, verbo Base Ancorada, que já responde pelo lado defensivo daquele vetor. Cenário sem essas duas perícias simplesmente não produz a cena em que a cedência importaria, e o Taichi resolve sozinho.

Contra o Jiu-Jitsu, a distância é o destino do corpo alheio. O Jiu-Jitsu projeta o oponente até o chão. O Taichi devolve a força dele para ele mesmo, sem precisar projetar coisa nenhuma. Contra Aikido, a distância é o que sobra depois do golpe. O Aikido usa a trajetória que o atacante já trouxe e não guarda nada dela. O Taichi absorve, comprime e pode guardar o que absorveu para liberar depois, em outro instante e contra outro alvo.

O regente é DES porque a identidade do domínio é sensibilidade de contato e não velocidade de deslocamento nem força de músculo. A régua de níveis desta perícia lê a população dos praticantes formados, e o LV1 já é o corpo que aprendeu a ceder em vez de resistir.

═══════════════════════════════════════════════════════════════════

IDENTIDADE

Regente DES. Categoria Físico, multiplicador 1,0. Camada superclasse, sem portão sobre nenhuma outra perícia. O praticante opera o próprio corpo contra o corpo alheio, sem converter nem manipular chakra em caminho algum, e a energia bruta que a categoria Chakra mediria não entra nesta perícia em nenhuma forma.

LV1 900 · LV2 1.350 · LV3 2.700 · LV4 4.950 · LV5 8.100 · LV6 12.150, acumulando 30.150. Caminho extra custa 2.475 no LV4 e 4.050 no LV5.

REQUISITOS DE ACESSO

O domínio não tem porta narrativa própria. Qualquer personagem com professor disposto e tempo de treino alcança o LV1, e o que separa o praticante comum do praticante de ápice é tempo de mesa e não limiar de ficha.

═══════════════════════════════════════════════════════════════════

PORTÃO DE RANK

O portão soma a Base Total ao atributo regente.

LV1 · Base 12 · DES 2 · portão 14 · rank E
LV2 · Base 24 · DES 6 · portão 30 · rank D
LV3 · Base 32 · DES 12 · portão 44 · rank C
LV4 · Base 40 · DES 18 · portão 58 · rank B
LV5 · Base 45 · DES 26 · portão 71 · rank A
LV6 · Base 50 · DES 34 · portão 84 · rank S

COMO O TESTE SE LÊ

teste = Base Total + DES + inclinação, se a situação pertence a ela + especialização, se o verbo resolve o problema

Uma inclinação e uma especialização por teste, no máximo.

RESOLUÇÃO POR OPOSIÇÃO

Todo verbo desta perícia que confronta o corpo alheio resolve pelo motor de manobras do Núcleo de Combate, Seção 9, com o teste desta perícia entrando onde o motor pede o atributo do executor. Nenhum efeito resolve por dificuldade arbitrada.

═══════════════════════════════════════════════════════════════════

A EXCEÇÃO DE ENRAIZAMENTO

Esta seção existe separada do caminho que a possui porque é a primeira substituição dura de atributo por atributo dentro de um teste em todo o corpus. Nenhum efeito publicado antes desta perícia troca um atributo por outro de forma automática e mecânica. O precedente mais próximo é narrativo: a vantagem Gênio Bruto autoriza o jogador a propor durabilidade no lugar de força, sujeita ao aval do Mestre e limitada a três usos por sessão. O que Enraizamento publica é diferente em forma, permanente enquanto o caminho estiver aberto e sem arbitragem de mesa.

Contra Empurrar e Derrubar dirigidos ao praticante, o par defensivo publicado pelo Núcleo de Combate lê RES no lugar do segundo termo:

```
Empurrar   FOR + VEL   contra   DEF + RES
Derrubar   FOR + VEL   contra   DEF + RES
```

A troca vale apenas nestas duas linhas e apenas quando o praticante é o alvo da manobra. Agarrar e Desarmar seguem o par publicado sem alteração, porque nenhum dos dois lê estrutura de apoio contra o chão. A razão é canônica ao próprio conceito do domínio: a raiz do Taichi é a estrutura montada que conduz a força recebida até o solo, e é a resistência da estrutura, não a força do músculo nem a rapidez do reflexo, que decide se o corpo sai do lugar.

A vantagem Gênio Bruto e esta exceção não se somam. Um praticante com as duas aplica a de Enraizamento nas duas manobras que ela toca, porque ela já entrega o mesmo resultado sem custar uso de sessão.

═══════════════════════════════════════════════════════════════════

RÉGUAS TRANSVERSAIS

Escritas uma vez para toda a perícia. Cada grandeza lê a especialização de um único caminho, e o praticante que não possui aquele caminho lê o teste geral. Os valores publicados assumem a largura mínima em que cada efeito pode existir.

LIMIAR DE RAIZ

O ativo mínimo que uma manobra de Empurrar ou Derrubar precisa somar para deslocar o praticante. Lê a especialização de Enraizamento.

limiar = ⌊teste ÷ 2⌋

Fora do caminho: 7 no LV1, 15 no LV2, 22 no LV3, 29 no LV4, 35 no LV5, 42 no LV6. Com Enraizamento: 58 no LV4, 71 no LV5, 84 no LV6, e 88 na largura de ápice. Manobra cujo ativo, FOR mais VEL do executor, fique igual ou abaixo deste limiar não desloca nem derruba o praticante, qualquer que seja o resultado do teste oposto. Acima do limiar a manobra resolve normalmente, com a substituição de Enraizamento valendo do lado defensivo.

TETO DA RESERVA DE FORÇA

Volume que a Reserva de Força comporta antes de transbordar. Lê a especialização de Absorção Ativa. O excedente acima do teto se perde no ato, sem carregar.

teto = teste × 25

Fora do caminho: 350 no LV1, 750 no LV2, 1.100 no LV3, 1.450 no LV4, 1.775 no LV5, 2.100 no LV6. Com Absorção Ativa: 3.000 no LV4, 3.675 no LV5, 4.350 no LV6, e 4.550 na largura de ápice.

PISO DE ALAVANCA

Grau mínimo garantido do estado que Arrancar-Quebrar impõe, antes da margem de vitória posicionar o grau final dentro do range. Lê a especialização de Arrancar-Quebrar.

piso = ⌊teste ÷ 50⌋, teto Grave

O resultado da fórmula lê a régua de graus do Sistema de Efeitos de Status: 0 é nenhum piso garantido, 1 é Grau Leve, 2 é Grau Moderado, e 3 ou mais é Grau Grave, o teto desta régua. Fora do caminho: nenhum piso garantido até o LV3, Grau Leve do LV4 ao LV6. Com Arrancar-Quebrar: Grau Moderado no LV4, Grau Moderado no LV5, Grau Grave no LV6 e na largura de ápice.

PRESSÃO CURTA

Fator de Técnica que Cotovelo-Ombro soma no passo 7 da sequência de resolução do Núcleo de Combate. Lê a especialização de Cotovelo-Ombro.

pressão = ⌊teste × 1,5⌋

Fora do caminho: 21 no LV1, 45 no LV2, 66 no LV3, 87 no LV4, 106 no LV5, 126 no LV6. Com Cotovelo-Ombro: 174 no LV4, 213 no LV5, 253 no LV6, e 265 na largura de ápice.

PISO DE ESCUTA

Piso do lado defensivo do teste de conexão do praticante contra o corpo que ele está lendo em contato sustentado. Lê a especialização de Escuta.

piso = ⌊teste ÷ 2,5⌋, teto 95

Fora do caminho: 5 no LV1, 12 no LV2, 17 no LV3, 23 no LV4, 28 no LV5, 33 no LV6. Com Escuta: 46 no LV4, 56 no LV5, 67 no LV6, e 70 na largura de ápice.

FRAÇÃO DE DESCARGA

Fração da Reserva de Força acumulada que Fa Jing converte em Energia num único disparo. Lê a especialização de Liberação.

fração = teste ÷ 200, teto 1,00

Fora do caminho: 0,07 no LV1, 0,15 no LV2, 0,22 no LV3, 0,29 no LV4, 0,355 no LV5, 0,42 no LV6. Com Liberação: 0,60 no LV4, 0,735 no LV5, 0,87 no LV6, e 0,91 na largura de ápice.

CARGA DA RESERVA

Fração da Energia de um golpe recebido que a Reserva de Força acumula. Resolve por categoria, e não por número, porque a fonte de dano varia demais para uma única constante fechar contra golpe elemental, obra de rank alto e ataque comum ao mesmo tempo.

Golpe aparado ativamente, dentro de um Bloqueio contra golpe físico, carrega um quinto da Energia do golpe. Golpe evitado por Esquiva não carrega, porque a Esquiva bem-sucedida anula o golpe por completo e nenhuma Energia atravessa o contato. Golpe recebido sob o verbo Ceder, sem gastar Bloqueio naquele golpe específico, carrega metade da Energia dele. A Energia lida é a publicada pelo Núcleo de Combate, FOR somada a VEL e ao bônus geral da perícia ofensiva vezes o multiplicador do golpe declarado, ou o valor que a obra declarar quando a fonte for uma técnica.

DECAIMENTO DA RESERVA

Dentro de uma mesma cena de combate a Reserva de Força não decai. Ao fim de cada cena de combate, todo valor acumulado na Reserva se perde por completo, e o praticante entra na cena seguinte com a Reserva vazia.

═══════════════════════════════════════════════════════════════════

NÍVEL 1: FUNDAMENTOS
Base 12 · teste típico 14 com DES 2 · rank E

O praticante formado já enraíza a postura contra empurrão e derrubada rasos, com o limiar de raiz em 7. A Reserva de Força existe desde este nível, comporta 350 e carrega de todo golpe aparado em Bloqueio, ou pelo verbo Ceder quando ele estiver disponível. A descarga da Reserva sai a 7% do que estiver acumulado, custa uma Ação Completa e zera o que foi liberado, ficando disponível a qualquer praticante deste nível ainda que nenhum caminho de Liberação tenha sido comprado.

NÍVEL 2: INCLINAÇÃO (escolha 1)
Base 24 · Inclinação 8 · teste típico 30 fora e 38 dentro, com DES 6 · rank D

O limiar de raiz sobe a 15, o teto da Reserva a 750, e a descarga a 15% do acumulado. A inclinação escolhida entra em uso.

RAIZ: o que decide é resistir ao deslocamento e absorver força sem ceder posição. O praticante que enraizou o corpo devolve o que chega ao chão em vez de deixar que o desloque.
└─ Entra no teste quando a cena dirige deslocamento, desequilíbrio ou impacto direto ao corpo do praticante

PORTAS: o que decide é a alavanca aplicada em contato próximo, depois que o oponente já está comprometido. É controle de estrutura alheia, exercido perto o bastante para que a distância deixe de ser variável.
└─ Entra no teste quando o par está na banda Engajado e o oponente já gastou o espaço de recuo que a cena oferecia

CÍRCULO: o que decide é o contato sustentado, a leitura do outro e a liberação do que foi guardado. O praticante que mantém o vínculo através de mais de um instante lê o que o corpo alheio vai fazer antes de ele terminar de fazer.
└─ Entra no teste quando há contato sustentado por mais de um turno com o mesmo corpo

NÍVEL 3: COMPETÊNCIA
Base 32 · Inclinação 16 · teste típico 44 fora e 60 dentro, com DES 12 · rank C

O limiar de raiz sobe a 22, o teto da Reserva a 1.100, e a descarga a 22% do acumulado. O praticante lê o valor exato de Reserva acumulada antes de descarregar, e a descarga deixa de custar uma Ação Completa dedicada, correndo dentro do Esforço de um Golpe comum.

NÍVEL 4: ESPECIALIZAÇÃO (escolha 1 caminho)
Base 40 · Inclinação 20 · Especialização 38 a 42 · teste típico 58 fora, 78 dentro e 116 a 120 no caminho, com DES 18 · rank B

NÍVEL 5: MAESTRIA
Base 45 · Inclinação 22 · Especialização 49 a 54 · teste típico 71 fora, 93 dentro e 142 a 147 no caminho, com DES 26 · rank A

NÍVEL 6: TRANSCENDÊNCIA (escolha 1)
Base 50 · Inclinação 25 · Especialização 60 a 65 transcendida · DES 34 · rank S
Requisito: todos os caminhos possuídos no estado aprofundado

O requisito acima abre o nível, não escolhe a transcendência: cada uma nomeia, além dele, o pré-requisito próprio de caminhos que a compõem, e o portador só a escolhe possuindo-os Aprofundados.

Testes plenos no LV6 com largura mínima: grau 3 alcança 169, grau 4 alcança 174. As três inclinações têm a mesma forma, um caminho de grau 3 e um de grau 4 cada, e a largura de ápice é simétrica nas três: com a inclinação inteira possuída, o caminho de grau 3 entrega 177 e o de grau 4 entrega 182, lidos pela especialização transcendida do caminho que cada efeito declarar.

═══════════════════════════════════════════════════════════════════
INCLINAÇÃO RAIZ
═══════════════════════════════════════════════════════════════════

ENRAIZAMENTO (grau 3, complexidade 3) — 38
Manter o corpo onde ele está, conduzindo ao chão o que tentar tirá-lo de lá. A especialização deste caminho governa a régua de limiar de raiz.
└─ Postura [passivo, sem custo]: contra Empurrar e Derrubar dirigidos ao praticante, o par defensivo lê RES no lugar do segundo termo publicado, pela exceção declarada acima. LV5: a leitura vale também quando o praticante está desequilibrado, e não apenas de pé
└─ Ancoragem [reação, sem custo]: o praticante escolhe não se mover mesmo perdendo o teste de Empurrar contra ele, e a Energia do executor da manobra, FOR mais VEL dele vezes 25, carrega a Reserva de Força pela régua de Carga da Reserva na taxa de golpe aparado, em vez de produzir deslocamento. LV5: a mesma escolha se aplica a Derrubar, e o praticante permanece de pé carregando a Reserva pela mesma régua em vez de cair

ABSORÇÃO ATIVA (grau 4, complexidade 4) — 42
Guardar o que o corpo recebeu em vez de apenas dissipá-lo. A especialização deste caminho governa a régua de teto da Reserva de Força.
└─ Reserva de Força [passivo, sem custo]: todo golpe físico aparado em Bloqueio carrega a Reserva pela régua de Carga da Reserva, com o teto lendo este caminho, 3.000 contra os 1.450 de quem não o possui. LV5: 3.675, e a Reserva passa a carregar também de golpe absorvido à distância curta, quando a obra tocar o praticante sem contato direto
└─ Ceder [ativado, ação livre, declarado antes da resolução do golpe]: o praticante abre mão do Bloqueio contra um golpe específico, deixando o golpe passar por inteiro até a Absorção, e a carga daquele golpe na Reserva dobra pela régua de Carga da Reserva. LV5: o golpe cedido não impõe Quebra de Guarda, mesmo que o Impacto supere o que restava dela

═══════════════════════════════════════════════════════════════════
INCLINAÇÃO PORTAS
═══════════════════════════════════════════════════════════════════

ARRANCAR-QUEBRAR (grau 4, complexidade 4) — 42
As duas alavancas que usam a base do oponente contra ele mesmo, na banda Engajado. A especialização deste caminho governa a régua de piso de alavanca.
└─ Cai [ativado, ação completa, o Esforço de um Golpe]: na banda Engajado, puxa contra a direção que o oponente menos espera, resolvendo como a manobra Agarrar do Núcleo de Combate com o piso de alavanca garantindo ao menos o grau que a régua declara. LV5: o piso sobe um degrau na régua de status, sem alterar o teto
└─ Lie [ativado, ação completa, o Esforço de um Golpe]: na banda Engajado, torce a estrutura do oponente contra a própria linha de apoio dela, resolvendo como a manobra Derrubar do Núcleo de Combate com o mesmo piso garantido. LV5: a Queda resultante impõe também o grau Leve de Imobilização, enquanto o piso de alavanca sustentar Grau Moderado ou acima

COTOVELO-OMBRO (grau 3, complexidade 4) — 38
As armas de distância mínima, empregadas quando o oponente já perdeu o espaço para recuar. A especialização deste caminho governa a régua de pressão curta.
└─ Zhou [ativado, ação padrão, o Esforço de um Golpe]: na banda Engajado, golpe de cotovelo que soma a pressão curta como Fator de Técnica no passo 7 da sequência de resolução, sem coeficiente de entrega reduzido mesmo abaixo da Banda Mínima que um golpe comum sofreria. LV5: o Impacto do golpe ignora metade da redução que a Guarda do alvo aplicaria
└─ Kao [ativado, ação padrão, o Esforço de um Golpe]: na banda Engajado, golpe de ombro que soma a mesma pressão curta e, ao superar a Guarda restante do alvo, adiciona a Queda pela régua de status no lugar de apenas quebrar a Guarda. LV5: a Queda resultante garante o piso de alavanca do praticante, mesmo sem o caminho Arrancar-Quebrar

═══════════════════════════════════════════════════════════════════
INCLINAÇÃO CÍRCULO
═══════════════════════════════════════════════════════════════════

ESCUTA (grau 3, complexidade 4) — 38
Ting Jing, a leitura do outro pelo contato sustentado. A especialização deste caminho governa a régua de piso de escuta.
└─ Ting Jing [passivo, sem custo, contato sustentado]: o lado defensivo do teste de conexão do praticante contra o corpo em contato lê o piso de escuta quando esse piso for maior que o Limiar normal, e o praticante ganha vantagem na Reação seguinte contra qualquer ação física daquele corpo. LV5: a vantagem se estende à primeira Reação contra uma obra executada pelo mesmo corpo, e não apenas contra ação física
└─ Adesão [passivo, sem custo]: uma vez estabelecido o contato, ele se mantém através do movimento do oponente sem gastar ação nova, mesmo que o oponente se desloque dentro da banda Engajado. LV5: a Adesão sobrevive a um deslocamento até a banda Curta, exigindo apenas que o praticante gaste a Ação de Movimento para acompanhar

LIBERAÇÃO (grau 4, complexidade 5) — 42
Fa Jing, a descarga da Reserva de Força num único golpe. A especialização deste caminho governa a régua de fração de descarga.
└─ Fa Jing [ativado, ação padrão, o Esforço de um Golpe]: com a Reserva de Força acumulada, descarrega a fração declarada dela num único golpe, e a Energia do golpe lê Reserva multiplicada pela fração no lugar do par FOR mais VEL comum. A Reserva usada no disparo zera, e o que exceder o consumido permanece guardado até a próxima descarga ou o fim da cena. LV5: a fração sobe a 0,735, e o disparo ignora a Banda Mínima da distância de contato
└─ Devolução [reação, o Esforço de um Bloqueio, resposta direta a golpe ou agarrão sofrido no mesmo turno]: em vez de ler a Reserva, a Energia do golpe lê a Energia do golpe recebido que originou a resposta, e a Guarda do agressor não responde a esse golpe porque a estrutura dele está montada para a frente do próprio ataque. O eixo de evitabilidade é ignora-esquiva, porque o agressor já comprometeu a própria Reação no golpe que originou a resposta. LV5: a leitura se estende ao golpe mais forte que o praticante sofreu na cena corrente, e não apenas ao último

═══════════════════════════════════════════════════════════════════

NÍVEL 6: TRANSCENDÊNCIAS

Cada transcendência revoga uma regra nomeada do sistema, e os efeitos dos caminhos da própria composição ganham a versão LV6 declarada. As leituras aplicam a largura de ápice da inclinação correspondente.

KUNITOKOTACHI (remoção de condição · RAIZ)
Pré-requisito: Enraizamento e Absorção Ativa, ambos Aprofundados.
Revoga: força suficiente desloca qualquer corpo apoiado.
└─ Raiz Sem Fim [passivo, sem custo]: enquanto o praticante mantiver apoio no chão, nenhum Empurrar e nenhum Derrubar o desloca, qualquer que seja o ativo do executor. A comparação contra o limiar de raiz deixa de ocorrer, porque não existe mais força que a supere
└─ Postura LV6: a leitura de RES no lugar do segundo termo se torna irrelevante contra Empurrar e Derrubar, porque nenhum dos dois desloca o praticante de qualquer forma; contra as demais manobras a substituição segue valendo
└─ Ancoragem LV6: toda manobra dirigida ao praticante enraizado carrega a Reserva de Força na taxa dobrada de Ceder, mesmo sem declaração prévia

AME-NO-TAJIKARAO (elevação de escopo · PORTAS)
Pré-requisito: Arrancar-Quebrar e Cotovelo-Ombro, ambos Aprofundados.
Revoga: a manobra custa a Ação Completa e não causa dano.
└─ Alavanca que Fere [passivo, sem custo]: Cai, Lie, Zhou e Kao passam a resolver dentro do Esforço de um Golpe comum, e o estado que cada um impõe se soma ao dano do próprio golpe no mesmo ato, sem exigir uma segunda ação
└─ Cai LV6: o piso de alavanca sobe a Grave garantido, com o teto normal da régua de status acima dele
└─ Lie LV6: a Imobilização imposta junto da Queda nasce em Grau Moderado, e não mais Leve
└─ Zhou LV6: o Impacto do golpe ignora a Guarda por completo, e não apenas metade dela
└─ Kao LV6: a Queda resultante garante Grau Grave de piso de alavanca, mesmo sem o caminho Arrancar-Quebrar

ŌMONONUSHI (remoção de condição · CÍRCULO)
Pré-requisito: Escuta e Liberação, ambos Aprofundados.
Revoga: o teto da Reserva de Força, e a perda dela ao fim da cena.
└─ Vaso Sem Fundo [passivo, sem custo]: a Reserva de Força deixa de ter teto e atravessa o fim de cena sem se perder, permanecendo acumulada até que o praticante a descarregue por vontade própria
└─ Fa Jing LV6: a fração de descarga sobe a 0,91, e a Reserva usada num disparo parcial não zera, apenas reduz pelo valor consumido
└─ Devolução LV6: a leitura da Energia recebida se estende a qualquer golpe sofrido nas últimas três rodadas, e não apenas ao turno corrente
└─ Ting Jing LV6: o piso de escuta se aplica contra qualquer corpo dentro da banda Curta, dispensando o contato sustentado
└─ Adesão LV6: o vínculo sobrevive a qualquer deslocamento do oponente dentro da banda Média, sem custar a Ação de Movimento do praticante

═══════════════════════════════════════════════════════════════════

ARQUÉTIPOS DE ÁPICE

A árvore que não sai do lugar constrói Kunitokotachi sobre Enraizamento e Absorção Ativa. Nenhuma força a desloca, e o que chega até ela vira reserva para o golpe seguinte. A mesa dela administra terreno e paciência, porque perder tempo contra ela é entregar munição.

O punho das oito portas constrói Ame-no-Tajikarao sobre Arrancar-Quebrar e Cotovelo-Ombro. Ele fecha a distância uma vez e resolve tudo dentro dela, com cada alavanca virando dano no mesmo instante em que desequilibra. É o arquétipo que não precisa de uma segunda rodada de contato.

O ouvido que devolve constrói Ōmononushi sobre Escuta e Liberação. Ele carrega o que absorve por cenas inteiras e escolhe o instante do disparo, e a única defesa contra ele é nunca deixar a luta se estender o bastante para a reserva encher.

═══════════════════════════════════════════════════════════════════

ETIQUETAGEM

Canon: nenhuma. O Taichi não tem verbete no cânone de Naruto, e nenhum fundamento desta perícia vem da obra.

Atribuído: nenhum.

Cânone fraco declarado: não se aplica, pela ausência de fonte primária.

Decisão de sistema declarada: a publicação em worlds/common, primeira perícia jogável do corpus fora de um mundo específico, porque nenhum efeito dela lê fato de cenário e a dependência inteira mora em documentos de núcleo; a tradução da arte marcial real Taichi Chuan pelo método de quatro passos, com o pilar de Enraizamento, o pilar de Ceder e Redirecionar e o pilar de Escuta pelo Contato como os três eixos originais, e as Oito Portas do repertório real como a quarta camada que vira a inclinação PORTAS; a fronteira contra Aikido, que projeta sem guardar, e contra Jiu-Jitsu, que arremessa o corpo alheio em vez de devolver a força dele; o regente DES em vez do VEL que artes marciais de velocidade usam, porque a identidade do domínio é sensibilidade e não deslocamento; a Reserva de Força como pool separado de PV e PS, que decai por cena inteira e não por turno; a substituição de RES pelo segundo termo do par defensivo de Empurrar e Derrubar, isolada em seção própria por ser a primeira do tipo no corpus; a leitura da Energia do golpe recebido pelo verbo Devolução, sem precedente publicado antes desta perícia.

Fontes descartadas com registro: nenhuma, pela ausência de fonte primária a descartar.

Proposta: todos os valores de custo, teste, régua e teto; a partição em três inclinações e seis caminhos; as seis réguas transversais; a régua de níveis inteira; as três transcendências e a nomeação delas; os três arquétipos de ápice.

DEPENDÊNCIAS E PENDÊNCIAS

Núcleo de Combate v3, dono do motor de manobras, da Energia, da Guarda, da Absorção e das cinco bandas, consumido sem redefinição em nenhum ponto desta perícia. Sistema de Efeitos de Status, dono da Imobilização, da Queda e da régua de graus, consumido pelos verbos Cai, Lie e Kao. Compêndio de escola de combate, inexistente, dono de qualquer obra nomeada que um praticante desta perícia venha a executar. No cenário Naruto, a perícia de Controle de Chakra é dona do caminho Impregnação e do verbo Base Ancorada, com fronteira declarada contra Enraizamento pela separação de vetor.

FILA DE ERRATA

Núcleo de Combate v1, Seção 9: a Seção declara que Empurrar produz "deslocamento forçado, metros pela margem", e nenhum documento do corpus publica a conversão de margem em metros. Esta perícia não a inventa, e resolve o próprio efeito de Enraizamento por comparação de limiar em vez de por metros, conforme a Seção 12 do Manual de Criação de Perícias.

Núcleo de Combate v1, Seção 9: a Seção cita "a margem de vitória posiciona o grau do estado produzido pela escala de resultado já publicada" para produzir o grau de estado de uma manobra vencida. A escala de resultado do Núcleo do Sistema produz Sucesso Pleno, Sucesso Justo, Falha Justa e Falha Grave, sem os quatro graus de estado nem limiares numéricos. A régua que produz Grau Leve, Moderado, Grave e Crítico vive no Sistema de Efeitos de Status, sob o nome Regras Universais de Aplicação, e lê margem de falha do resistente, não margem de vitória do atacante. A citação da Seção 9 aponta para o documento errado, e nenhum limiar numérico de margem de vitória está publicado em lugar nenhum do corpus.

Núcleo do Sistema, ou onde a régua de margem de vitória vier a residir: publicar os limiares de margem de vitória em teste oposto, hoje ausentes, para que os verbos Cai, Lie e Kao desta perícia parem de depender do piso garantido como único ponto fixo.

Manual de Vantagens do Núcleo, vantagem Gênio Bruto, linha de Resistência: declarar explicitamente que a substituição narrativa que ela concede não se soma à exceção estrutural que esta perícia publica, para a mesma manobra e o mesmo par de atributos.

═══════════════════════════════════════════════════════════════════

REGISTRO DE ALTERAÇÕES

v1

• Documento inicial

v1.1

• Cada transcendência de LV6 passa a nomear, além do requisito geral de nível, a composição própria de caminhos Aprofundados que a compõe, conforme o Manual de Criação de Perícias v7.4 §13

═══════════════════════════════════════════════════════════════════

Estrutura e valores de camada leem o Núcleo de Perícias v2.2. Procedimento segue o Manual de Criação de Perícias v7.2. Custos leem o Manual de Economia de XP v2, seção 6.1. O motor de manobras, a Energia, a Guarda e a Absorção leem o Núcleo de Combate v3. A régua de graus de estado e as condições Imobilização e Queda pertencem ao Sistema de Efeitos de Status v2.4. No cenário Naruto, a fronteira contra Impregnação lê a perícia de Controle de Chakra v3.3.
