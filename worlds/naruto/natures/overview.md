---
id: naruto.natures.overview
title: "Naturezas de Chakra — Naruto"
version: 1
layer: scenario
scenario: naruto
type: system
status: final
source-file: _index.md
---

# Naturezas de Chakra — Naruto

Todo chakra possui uma natureza de transformação. A maioria dos shinobi tem **afinidade com uma natureza** (mais fácil de moldar) mas pode aprender outras com treinamento intenso.

## As 5 Naturezas Básicas

| Natureza | Nome JP | Arquivo | Pontos Fortes |
|---|---|---|---|
| [Fogo](basic/fire.md) | Katon | — | Poder ofensivo, pressão, área |
| [Vento](basic/wind.md) | Fūton | — | Corte, alcance, amplificação |
| [Relâmpago](basic/lightning.md) | Raiton | — | Velocidade, perfuração, precisão |
| [Terra](basic/earth.md) | Doton | — | Defesa, contenção, terreno |
| [Água](basic/water.md) | Suiton | — | Fluxo, adaptação, controle de campo |

## Ciclo de Relações
```
Fogo > Vento > Relâmpago > Terra > Água > Fogo
(cada natureza é fraca contra a seguinte no ciclo)
```

## Naturezas Avançadas (Kekkei Genkai)
Combinações de duas ou mais naturezas básicas. Ver [Naturezas Avançadas](./advanced.md).

## Dados Relacionados
- `src/shared/data/rulesets/naruto/nature_transformations.json`
