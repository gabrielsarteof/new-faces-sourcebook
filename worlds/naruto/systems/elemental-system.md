...
| Controle de Chakra, caminho Vazão Ampliada | O caminho publica `Limite de Saída = RC × (20% + 5% × √(teste de Condução ÷ 205))` com teto de vinte e cinco por cento. A base de vinte por cento deixou de ser universal, e o teto do caminho passou a coincidir com a largura que a Afinidade já recebe de graça. A régua precisa declarar se a ampliação incide sobre a largura da posição empregada e qual teto ela alcança, e a decisão é de desenho |
| Reencarnação de Indra e vantagem de vazão ampliada | Os dois elevam o Limite de Saída "de vinte para trinta por cento", ancorados na base universal revogada. O ponto de partida passa a depender da posição da natureza empregada |
